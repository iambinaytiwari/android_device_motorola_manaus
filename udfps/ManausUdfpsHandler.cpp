...
class ManausUdfpsHandler : public UdfpsHandler {
  public:
    ManausUdfpsHandler() : hbmFodEnabled(false), mMotoFingerprint(nullptr), mEgisFingerprint(nullptr) {
  ...
  private:
    ...
    sp<IBiometricsFingerprintRbs> getEgisFingerprint() {
        if (!mEgisFingerprint) {
            mEgisFingerprint = IBiometricsFingerprintRbs::tryGetService();
            if (!mEgisFingerprint) {
                LOG(WARNING) << "Failed to get IBiometricsFingerprintRbs service via tryGetService";
            }
        }
        return mEgisFingerprint;
    }

    void extraApiWrapper(int cidValue) {
        auto egisFingerprint = getEgisFingerprint();
        if (!egisFingerprint) return;
        ...
        egisFingerprint->extra_api(7, hidl_cid, [](const ::android::hardware::hidl_vec<uint8_t>&){});
    }
    ...
    sp<IBiometricsFingerprintRbs> mEgisFingerprint;
};
